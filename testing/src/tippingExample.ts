...
async function main() {
  ...
  const config: WarpConfig = {
    ...
    userAddress:
      "erd1qyu5wthldzr8wx5c9ucg8kjagg0jfs53s8nr3zpz3hypefsdd8ssycr6th", // REQUIRED for inscription tx
  };
  ...
  try {
    ...
    if (shouldCreateInscription) {
      ...
      console.log(
        "To create an inscription transaction, set shouldCreateInscription to true"
      );
    }

    console.log("\nNext steps:");
    console.log(
      "1. Set shouldCreateInscription to true to generate a transaction"
    );
