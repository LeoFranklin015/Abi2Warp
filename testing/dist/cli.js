...
const publishWarp_1 = require("./publishWarp");
...
function printRecipientLinks(result) {
    if (result.recipientLinks && Object.keys(result.recipientLinks).length > 0) {
        ...
        Object.entries(result.recipientLinks).forEach(([recipient, link]) => {
            console.log(`  ${recipient.slice(0, 8)}...${recipient.slice(-6)}: ${link}`);
        });
    }
}
...
if (walletFile && walletPassword) {
    ...
    (0, publishWarp_1.publishWarpWithWalletFile)(walletFile, walletPassword, network, recipients)
        .then((result) => {
    ...
    (0, publishWarp_1.publishWarp)(privateKey, network, recipients)
        .then((result) => {
        console.log(result);
        // printSuccess(result.warpUrl, network);
        // printRecipientLinks(result);
...
function printSuccess(url, network) {
    console.log(`
========================================
