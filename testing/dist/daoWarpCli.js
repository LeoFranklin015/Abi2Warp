...
const createDaoVoteWarp_1 = require("./createDaoVoteWarp");
async function main() {
...
`;
    ...
    try {
        if (type === "vote" || type === "all") {
            console.log("\nCreating VOTE warp...");
            const { warpUrl } = await (0, createDaoVoteWarp_1.createDaoVoteWarp)(contract, wallet, password, network);
            console.log(`✅ Vote warp created: ${warpUrl}`);
        }
        if (type === "view" || type === "all") {
            console.log("\nCreating VIEW warp...");
            const { warpUrl } = await (0, createDaoVoteWarp_1.createDaoViewWarp)(contract, wallet, password, network);
            console.log(`✅ View warp created: ${warpUrl}`);
        }
        if (type === "propose" || type === "all") {
            console.log("\nCreating PROPOSE warp...");
            const { warpUrl } = await (0, createDaoVoteWarp_1.createDaoProposalWarp)(contract, wallet, password, network);
            console.log(`✅ Propose warp created: ${warpUrl}`);
            console.log(`⚠️ Note: Only the contract owner can use this warp to create proposals!`);
        }
        if (type === "execute" || type === "all") {
            console.log("\nCreating EXECUTE warp...");
            const { warpUrl } = await (0, createDaoVoteWarp_1.createDaoExecuteWarp)(contract, wallet, password, network);
            console.log(`✅ Execute warp created: ${warpUrl}`);
            console.log(`⚠️ Note: Only the contract owner can use this warp to execute proposals!`);
        }
