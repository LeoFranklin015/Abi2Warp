...
exports.createDaoVoteWarp = createDaoVoteWarp;
exports.createDaoViewWarp = createDaoViewWarp;
exports.createDaoProposalWarp = createDaoProposalWarp;
exports.createDaoExecuteWarp = createDaoExecuteWarp;
...
const walletUtils_2 = require("./walletUtils");
/**
 ...
 */
async function createDaoVoteWarp(contractAddress = "erd1qqqqqqqqqqqqqpgqa2h4d6rufxlu76l3zx5xdsh2suk94ynsldvsr8nusj", walletPath = "./src/leo.json", password = "Leo@2003", network = "devnet") {
    ...
    builder
        ...
        url: `https://${network === "mainnet" ? "" : network + "-"}explorer.multiversx.com/accounts/${contractAddress}`,
    });
    // Build the warp
    ...
    const txHash = await (0, walletUtils_2.signAndBroadcastTransaction)(tx, signer, account, network);
    // Generate warp URL
...
async function createDaoViewWarp(contractAddress = "erd1qqqqqqqqqqqqqpgqa2h4d6rufxlu76l3zx5xdsh2suk94ynsldvsr8nusj", walletPath = "./src/leo.json", password = "Leo@2003", network = "devnet") {
    // Initialize API provider
    const apiUrl = network === "mainnet"
        ? "https://api.multiversx.com"
        : `https://${network}-api.multiversx.com`;
 ...
 * @returns The created warp and transaction hash
 */
async function createDaoProposalWarp(contractAddress = "erd1qqqqqqqqqqqqqpgqa2h4d6rufxlu76l3zx5xdsh2suk94ynsldvsr8nusj", walletPath = "./src/leo.json", password = "Leo@2003", network = "devnet") {
    // Initialize API provider
    const apiUrl = network === "mainnet"
        ? "https://api.multiversx.com"
        : `https://${network}-api.multiversx.com`;
    ...
    builder
        ...
        .setDescription("Create a new proposal in the DAO (only for contract owner)")
    ...
    console.log(`Warp URL: ${warpUrl}`);
    console.log(`Transaction: https://${network === "mainnet" ? "" : network + "-"}explorer.multiversx.com/transactions/${txHash}`);
    console.log(`⚠️ Note: Only the contract owner can create proposals with this warp!`);
    console.log(`======================================\n`);
    return { warp, txHash, warpUrl };
}
// Also create a warp for executing proposals (owner only)
async function createDaoExecuteWarp(contractAddress = "erd1qqqqqqqqqqqqqpgqa2h4d6rufxlu76l3zx5xdsh2suk94ynsldvsr8nusj", walletPath = "./src/leo.json", password = "Leo@2003", network = "devnet") {
    // Initialize API provider
    const apiUrl = network === "mainnet"
        ? "https://api.multiversx.com"
        : `https://${network}-api.multiversx.com`;
    ...
    builder
        ...
        .setDescription("Execute a proposal after voting period (only for contract owner)")
    ...
    console.log(`Warp URL: ${warpUrl}`);
    console.log(`Transaction: https://${network === "mainnet" ? "" : network + "-"}explorer.multiversx.com/transactions/${txHash}`);
    console.log(`⚠️ Note: Only the contract owner can execute proposals with this warp!`);
    console.log(`======================================\n`);
    return { warp, txHash, warpUrl };
}
// Main function
async function main() {
    try {
        ...
        if (operation === "vote") {
            ...
            await createDaoProposalWarp(contractAddress, walletPath, password, network);
        }
        else if (operation === "execute") {
            // Create execute warp
            await createDaoExecuteWarp(contractAddress, walletPath, password, network);
        }
            ...
            console.error(`Unknown operation: ${operation}. Use 'vote', 'view', 'propose', 'execute', 'both', or 'all'`);
            process.exit(1);
        }
    }
    catch (error) {
