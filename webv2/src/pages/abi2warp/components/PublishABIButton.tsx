...
import { publishABI } from '../lib/PublishABI';
import { useTransactionHash } from '../hooks/useTransactionHash';

interface PublishABIButtonProps {
  abi: any;
  disabled?: boolean;
  className?: string;
...
export const PublishABIButton = ({
  abi,
  disabled,
  className,
  setTxHash
}: PublishABIButtonProps) => {
  ...
  const handlePublish = async () => {
    try {
      const result = await publishABI(abi, address);
      setSessionId(result.sessionId);
    } catch (error) {
      console.error('Error publishing ABI:', error);
    }
  };
  ...
  return (
    <div>
      ...
      >
        {isLoading ? 'Publishing...' : 'Publish ABI'}
      </button>
